  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F7F7F7] dark:bg-[#1a1a1a]">
      ...
      <AnimatePresence>
        {drawerOpen && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 bg-black bg-opacity-40 flex xl:hidden"
          >
            <motion.div 
              initial={{ x: '-100%' }}
              animate={{ x: 0 }}
              ...
              className="w-64 bg-white dark:bg-[#2a2a2a] h-full shadow-lg p-6 flex flex-col"
            >
              <button 
                onClick={() => setDrawerOpen(false)} 
                className="self-end mb-4 p-2 hover:bg-gray-100 dark:hover:bg-[#3a3a3a] rounded-full transition-colors"
                aria-label="Close menu"
              >
                <XMarkIcon className="w-6 h-6 text-[#E73828]" />
              </button>
              <AccountSidebar onClose={() => setDrawerOpen(false)} />
            </motion.div>
      ...
      <div className="flex w-full max-w-[1400px] mx-auto bg-white dark:bg-[#2a2a2a] rounded-[32px] border border-[#E73828]/10 shadow-[0_4px_24px_0_rgba(0,0,0,0.04)] flex-col xl:flex-row mt-16 xl:mt-8">
        ...
        <aside className="hidden xl:flex w-[300px] flex-shrink-0 border-r border-[#E73828]/10 p-8 justify-end bg-white dark:bg-[#2a2a2a] rounded-l-[32px]">
          <AccountSidebar />
        </aside>
